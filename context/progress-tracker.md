...
- **12-history-versioning**: Replaced flat `HistoryEntry[]` model with session+version model. Added `TicketVersion` and `TicketSession` types; removed `HistoryEntry`. Rewrote Zustand store — `generate()` success always creates a new `TicketSession` with v1 (no longer writes to history on `newTicket()`); `regenerate()` appends a new `TicketVersion` to the current session (max 4 versions); `newTicket()` just clears workspace state; added `navigateVersion("prev"|"next")` to traverse versions within a session; `restore(session)` restores input, template, and active version output. Added `activeSessionId` and `activeVersionId` to store state. LocalStorage key changed to `"devflow-sessions"` (persists sessions array only). Updated sidebar to render `TicketSession` list with active highlight. Added version navigator bar to output panel (shown when session has >1 version); Regenerate button disabled at version limit of 4. Build passes cleanly.
